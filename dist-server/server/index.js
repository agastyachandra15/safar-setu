...
function addEvent(trip, kind, title, detail) {
...
function activateFallback(trip) {
    if (trip.status !== 'grace')
        return;
    ...
    const cluster = nearLastKnown.filter((passenger) => nearLastKnown.every((other) => distanceMeters(passenger.location, other.location) <= 250));
    if (cluster.length < 2) {
        addEvent(trip, 'validation', 'Cluster not verified', 'Waiting for at least two consistent passengers within 250 m');
        emitState();
        return;
    }
    const centroid = point(cluster.reduce((sum, passenger) => sum + passenger.location.lat, 0) / cluster.length, cluster.reduce((sum, passenger) => sum + passenger.location.lng, 0) / cluster.length);
...
app.post('/api/trips/:id/action', (req, res) => {
    ...
    if (action === 'fail') {
        ...
        if (trip.status === 'fallback') {
            ...
            if (cluster.length > 0) {
                trip.currentLocation = point(cluster.reduce((sum, p) => sum + p.location.lat, 0) / cluster.length, cluster.reduce((sum, p) => sum + p.location.lng, 0) / cluster.length);
            }
        }
        else {
...
io.on('connection', (socket) => {
    ...
    socket.on('join-trip', (id) => socket.join(id));
});
